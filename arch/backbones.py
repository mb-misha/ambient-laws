...
class Mamba2Layer(nn.Module):
    def __init__(self, d_model, d_state=128, dropout=0.0, **kwargs):
        super().__init__()
        self.mamba2 = Mamba2(d_model=d_model, d_state=d_state, headdim=16, **kwargs)
        self.norm = nn.LayerNorm(d_model)

    def forward(self, x):
        """
        Inputs:
            x: (B, d_model, L) -- Batch size, embedding dimension, sequence length
        Returns:
            x: Processed tensor with the same shape as input
        """
        # Permute for Mamba compatibility
        x = x.transpose(-1, -2)  # (B, L, d_model)
        x = self.mamba2(x)
        x = self.norm(x)  # Back to (B, d_model, L)
        return x.transpose(-1, -2)

class Mamba2Block(nn.Module):
    def __init__(self, d_model, dropout=0.0, num_features=0, **kwargs):
        super().__init__()
        self.mamba2_layer = Mamba2Layer(d_model, dropout=dropout, **kwargs)
        self.time_linear = nn.Linear(d_model, d_model)
        self.tanh = nn.Tanh()
        self.sigm = nn.Sigmoid()
        self.out_linear1 = nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=1)
        self.out_linear2 = nn.Conv1d(in_channels=d_model, out_channels=d_model, kernel_size=1)
        self.feature_encoder = nn.Conv1d(num_features, d_model, kernel_size=1)

    def forward(self, x, t, features=None):
        t = self.time_linear(t)[:, None, :].repeat(1, x.shape[2], 1)
        t = t.transpose(-1, -2)
        out = self.mamba2_layer(x + t)
        if features is not None:
            out = out + self.feature_encoder(features)
        out = self.tanh(out) * self.sigm(out)
        out1 = self.out_linear1(out)
...
class BackboneModel(nn.Module):
    def __init__(
        ...
        elif residual_block == "mamba":
            residual_block = Mamba2Block
        else:
    ...
    def forward(self, input, t, features=None, **kwargs):
