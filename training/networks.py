...
from arch.backbones import BackboneModel
...
@persistence.persistent_class
class S4EDMPrecond(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self,
                 label_dim=0,  # Number of class labels, 0 = unconditional.
                 use_fp16=False,  # Execute the underlying model at FP16 precision?
                 sigma_min=0,  # Minimum supported noise level.
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        F_x = self.model((c_in.view(c_in.shape[0], 1, 1) * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
        assert F_x.dtype == dtype
        D_x = c_skip.view(c_skip.shape[0], 1, 1) * x + c_out.view(c_out.shape[0], 1, 1) * F_x.to(torch.float32)
        return D_x
    ...
    def generate(self, batch_size=1, device="cuda",
        ...
        net = self.to(device)
        # Pick latents and labels.
        latents = torch.randn([batch_size, 200, 1], device=device)
        class_labels = None
        if net.label_dim:
