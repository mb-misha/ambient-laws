...
class Conv1d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            x = torch.nn.functional.conv_transpose1d(x, f.mul(2).tile([self.in_channels, 1, 1]), groups=self.in_channels, stride=2, padding=max(f_pad - w_pad, 0))
            ...
            if self.up:
                x = torch.nn.functional.conv_transpose1d(x, f.mul(2).tile([self.in_channels, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad)
            if self.down:
...
#----------------------------------------------------------------------------
@persistence.persistent_class
class UNetBlock1D(torch.nn.Module):
    def __init__(self,
...
#----------------------------------------------------------------------------
@persistence.persistent_class
class SongUNet1D(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self,
        data_length,                       # Data length at input/output
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res}_conv'] = Conv1d(in_channels=cin, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res}_down'] = UNetBlock1D(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res}_aux_down'] = Conv1d(in_channels=caux, out_channels=caux, kernel=0, down=True, resample_filter=resample_filter)
                    self.enc[f'{res}_aux_skip'] = Conv1d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res}_aux_residual'] = Conv1d(in_channels=caux, out_channels=cout, kernel=3, down=True, resample_filter=resample_filter, fused_resample=True, **init)
                    caux = cout
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if decoder_type == 'skip' or level == 0:
                ...
                self.dec[f'{res}_aux_conv'] = Conv1d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
...
class EDMPrecond(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self,
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.model = globals()[model_type](data_length=img_resolution, in_channels=img_channels, out_channels=img_channels, label_dim=label_dim, **model_kwargs)
