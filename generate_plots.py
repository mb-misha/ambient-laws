...
def process_data(generated_filepath, normalization, stochastic_model, mu, sigma, theta, v0, n_steps, return_log_returns, n_training_paths, noise_sigma, n_paths=100000, K=110, **kwargs):
    ...
    with PdfPages(output_filename) as pdf:
        ...
        axes[0].axvline(real_var_mean, color='blue', linestyle='--', label=f'real var mean ({real_var_mean:.2f})')
        axes[0].axvline(generated_var_mean, color='orange', linestyle='--', label=f'generated var mean ({generated_var_mean:.2f})')
        axes[0].legend()
        ...
        axes[1].axvline(real_cvar_mean, color='blue', linestyle='--', label=f'real cvar mean ({real_cvar_mean:.2f})')
        axes[1].axvline(generated_cvar_mean, color='orange', linestyle='--', label=f'generated cvar mean ({generated_cvar_mean:.2f})')
        axes[1].legend()
        ...
        axes[0].axvline(mu_real_mean, color='blue', linestyle='--', label=f'real mu mean ({mu_real_mean:.2f})')
        axes[0].axvline(mu_gen_mean, color='orange', linestyle='--', label=f'generated mu mean ({mu_gen_mean:.2f})')
        axes[0].legend()
        ...
        axes[1].axvline(sigma_real_mean, color='blue', linestyle='--', label=f'real sigma mean ({sigma_real_mean:.2f})')
        axes[1].axvline(sigma_gen_mean, color='orange', linestyle='--', label=f'generated sigma mean ({sigma_gen_mean:.2f})')
        axes[1].legend()
